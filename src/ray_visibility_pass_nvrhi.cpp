...
static_assert(offsetof(DirectionalRayVisibilityConstants, directionToLightAndDistance) == sizeof(RendererViewConstants));
static_assert(offsetof(RayTracedFlashlightShadowConstants, lightPositionAndRange) == sizeof(RendererViewConstants));
static_assert(offsetof(RayTracedFlashlightShadowConstants, beamProfile) == sizeof(RendererViewConstants) + 32u);
static_assert(offsetof(RayTracedSkyVisibilityConstants, sampleSequencePhase) == sizeof(RendererViewConstants));
...
namespace uvsr
{
    namespace
    {
        ...
        const char* VisibilityName(RayVisibilityPass::Kind kind)
        {
            switch (kind)
            {
            case RayVisibilityPass::Kind::Sun: return "Directional Ray Visibility/Visibility";
            case RayVisibilityPass::Kind::Flashlight: return "Ray Traced Flashlight Shadows/Visibility";
            case RayVisibilityPass::Kind::Sky: return "Ray Traced Sky Visibility/Visibility";
            }
            return "Ray Visibility/Visibility";
        }
        ...
        bool IsFiniteFloat3(gpu_contract::Float3 value)
        {
        ...
        : m_Device(device), m_Kind(kind), m_BindlessLayout(bindlessLayout)
    {
            ...
            kind == Kind::Flashlight ? "uvsr/ray_traced_flashlight_shadows_cs.hlsl" : "uvsr/ray_traced_sky_visibility_cs.hlsl";
        {
            ...
            const auto shader = shaderFactory->CreateShader(file, entry, {}, nvrhi::ShaderType::Compute);
        ...
        nvrhi::ITexture* noise, nvrhi::ITexture* attemptMask)
    {
        ...
        if (!output || output->getDesc().width != depth.width || output->getDesc().height != depth.height)
        {
            ...
            description.debugName = VisibilityName(m_Kind);
    ...
    void RayVisibilityPass::Dispatch(nvrhi::ICommandList* commandList, const RendererView& view,
        const LightingSurfaceView& surface, const RaySceneView& rayScene)
    {
        ...
        commandList->dispatch(
            extent.width() / 8 + (extent.width() % 8 != 0),
            extent.height() / 8 + (extent.height() % 8 != 0));
        commandList->endMarker();
    }

    DirectionalRayVisibilityResult RayVisibilityPass::RenderDirectional(nvrhi::ICommandList* commandList,
        const DirectionalShadowSettings& settings, const RendererView& view, const LightingSurfaceView& surface,
        const RaySceneView& rayScene, const RendererSceneView& scene, RendererSceneHandle light, float sceneDiagonal,
    ...
    {
        if (m_Kind != Kind::Sun || !m_Supported || !settings.enabled || !commandList || !view.valid || !rayScene ||
            !light || scene.generation != rayScene.sceneGeneration || !IsDirectionalShadowSettingsValid(settings) ||
        ...
        if (!record || record->kind != RendererSceneLightKind::Directional ||
            !GetRendererSceneLightFrame(scene, light, frame).Succeeded()) return {};
        const float rayDistance = ResolveRayVisibilityMaxDistance(settings.maxDistance, sceneDiagonal);
        const gpu_contract::Float3 propagationDirection{
            float(frame.direction[0]), float(frame.direction[1]), float(frame.direction[2]) };
        const float directionLengthSquared = propagationDirection.x * propagationDirection.x +
            propagationDirection.y * propagationDirection.y + propagationDirection.z * propagationDirection.z;
        if (!std::isfinite(rayDistance) || !(rayDistance > 0.f) || !std::isfinite(directionLengthSquared) ||
            !(directionLengthSquared > 1.e-12f) || !std::isfinite(record->values.angularSize))
            return {};
        ...
        const gpu_contract::Float3 directionToLight{
            -propagationDirection.x / directionLength,
            -propagationDirection.y / directionLength,
            -propagationDirection.z / directionLength };
        ...
        constants.reverseDepth = view.reverseDepth ? 1u : 0u;
        ...
        constants.angularDiameter = ResolveShadowEmitterSize(
            std::clamp(record->values.angularSize, 0.f, 90.f) * 3.141592654f / 180.f, settings.hardShadows);
    ...
    RayTracedFlashlightShadowResult RayVisibilityPass::RenderFlashlight(nvrhi::ICommandList* commandList,
        const RendererView& view, const LightingSurfaceView& surface, const RaySceneView& rayScene,
        const RendererSceneView& scene, RendererSceneHandle light, const FlashlightBeamProfile& beamProfile, const NoiseSettings& noiseSettings,
    ...
    {
        if (m_Kind != Kind::Flashlight || !m_Supported || !commandList || !view.valid || !rayScene || !light || scene.generation != rayScene.sceneGeneration ||
            !noiseTexture || !sampleSchedule || sampleCount < 1u || sampleCount > 64u)
            return {};
        const auto* record = FindRendererSceneLight(scene, light);
        RendererSceneLightFrame frame;
        if (!record || record->kind != RendererSceneLightKind::Spot ||
            !GetRendererSceneLightFrame(scene, light, frame).Succeeded()) return {};
        const auto& values = record->values;
        const gpu_contract::Float3 lightPosition{
            float(frame.position[0]), float(frame.position[1]), float(frame.position[2]) };
        gpu_contract::Float3 lightDirection{
            float(frame.direction[0]), float(frame.direction[1]), float(frame.direction[2]) };
        const float directionLengthSquared = lightDirection.x * lightDirection.x +
        ...
        if (!FlashlightBeamProfileIsValid(beamProfile) ||
            ...
            rayBiasMeters > RayTracedFlashlightMaximumRayBias || !std::isfinite(values.range) ||
            !(values.range > beamProfile.emitterRadiusMeters) || !std::isfinite(values.radius) ||
            std::abs(values.radius - beamProfile.emitterRadiusMeters) > 1e-6f || !IsFiniteFloat3(lightPosition) ||
        ...
        const float directionLength = std::sqrt(directionLengthSquared);
        lightDirection.x /= directionLength;
        lightDirection.y /= directionLength;
        lightDirection.z /= directionLength;
    ...
    RayTracedSkyVisibilityResult RayVisibilityPass::RenderSky(nvrhi::ICommandList* commandList,
        const RayTracedSkyVisibilitySettings& settings, const RendererView& view, const LightingSurfaceView& surface,
    ...
    {
        if (m_Kind != Kind::Sky || !m_Supported || !commandList || !view.valid || !rayScene)
            return {};
        const float rayDistance = ResolveRayVisibilityMaxDistance(settings.maxDistance, sceneDiagonal);
