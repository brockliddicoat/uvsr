...
namespace uvsr
{
    ...
    struct MaterialInspectorLayout
    {
        float panelMinX = 0.f;
        float panelMinY = 0.f;
        float panelWidth = 0.f;
        float panelMaxHeight = 0.f;
    };
    ...
    [[nodiscard]] constexpr float AdvanceMaterialInspectorAppearance(
        float appearance,
        ...
        float deltaTimeSeconds)
    {
        return motionEnabled
            ? AdvancePixelZoomVisibility(
                ...
                requestedVisible,
                deltaTimeSeconds)
            : requestedVisible ? 1.f : 0.f;
    }

    [[nodiscard]] constexpr bool IsMaterialInspectorPresentationActive(
        bool requestedVisible,
        float appearance)
    {
        return requestedVisible || appearance > 0.f;
    }

    [[nodiscard]] constexpr bool IsMaterialInspectorAppearanceIdle(
        bool requestedVisible,
        float appearance)
    {
    ...
    // While the inspector is visible, Amp moves it onto zoom's minimum-scale
    // rail before zoom fades in, and keeps it there until zoom has completely
    // faded out. That sequencing prevents two floating panels from crossing.
    [[nodiscard]] constexpr float
        AdvanceMaterialInspectorZoomPlacement(
            float placement,
            bool inspectorPresented,
            bool zoomRequested,
            bool zoomPresentationActive,
            bool motionEnabled,
            float deltaTimeSeconds)
    {
        if (!motionEnabled || !inspectorPresented)
            return zoomRequested ? 1.f : 0.f;
        if (!zoomRequested && zoomPresentationActive)
            return 1.f;
        return AdvancePixelZoomVisibility(
            placement,
            zoomRequested,
            deltaTimeSeconds);
    }
    ...
    [[nodiscard]] constexpr bool
        ShouldDelayPixelZoomForMaterialInspector(
            ...
            float placement)
    {
        return inspectorPresented &&
            zoomRequested &&
            motionEnabled &&
            placement < 1.f;
    }

    // The inspector follows the presented zoom window's animated lower edge
    // while retaining the zoom window's full resting width and right rail.
    // During the invisible pre/post shift it interpolates to the same vertical
    // endpoint, so the first and last shared frames have no gap pop.
    [[nodiscard]] constexpr MaterialInspectorLayout
        ResolveMaterialInspectorLayout(
            ...
            float zoomPlacement,
            bool zoomPresentationActive,
            float zoomPresentationOpacity,
            float zoomLevelTransitionScale)
    {
        const PixelZoomLayout finalZoomLayout = ResolvePixelZoomLayout(
            sourceWidth,
            sourceHeight,
            panelMargin,
            PixelZoomMode::Zoom2x);
        const float easedPlacement =
            SmoothPixelZoomVisibility(zoomPlacement);
        const PixelZoomLayout initialZoomLayout =
            ResolveAnimatedPixelZoomLayout(
                finalZoomLayout,
                0.f,
                1.f);
        const PixelZoomLayout presentedZoomLayout =
            zoomPresentationActive
                ...
                : ResolveAnimatedPixelZoomLayout(
                    ...
                    1.f - easedPlacement,
                    1.f);
        const float initialBelowZoomY =
            float(
                initialZoomLayout.panelMinY +
                initialZoomLayout.panelHeight +
                panelMargin);

        MaterialInspectorLayout result;
        result.panelMinX =
            float(finalZoomLayout.panelMinX);
        result.panelMinY = zoomPresentationActive
            ? float(
                presentedZoomLayout.panelMinY +
                presentedZoomLayout.panelHeight +
                panelMargin)
            : float(panelMargin) +
                easedPlacement *
                    (initialBelowZoomY - float(panelMargin));
        result.panelWidth =
            float(finalZoomLayout.panelWidth);
        result.panelMaxHeight =
            sourceHeight > panelMargin &&
                    float(sourceHeight - panelMargin) > result.panelMinY
                ? float(sourceHeight - panelMargin) - result.panelMinY
                : 0.f;
        return result;
    }
