...
#else
#if CMAA2_CLAMP_SOURCE_LOADS_TO_VIEWPORT
    uint width, height;
    g_inoutColorReadonly.GetDimensions( width, height );
    int2 clampedPixelPos = clamp(
        int2( pixelPos ) + offset,
        int2( 0, 0 ),
        int2( width, height ) - 1 );
    lpfloat3 color = g_inoutColorReadonly.Load(
        int3( clampedPixelPos, 0 ) ).rgb;
#else
