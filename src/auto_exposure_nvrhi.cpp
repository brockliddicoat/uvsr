...
namespace uvsr
{
        ...
        bool diagnosticView)
    {
        ...
        if (!commandList || !frameView.valid)
        ...
        
        {
            ...
            constants.viewOrigin = {
                uint32_t(std::max(extent.minX, 0)),
                uint32_t(std::max(extent.minY, 0)) };
            constants.viewSize = {
                uint32_t(std::max(extent.width(), 0)),
                uint32_t(std::max(extent.height(), 0)) };
            ...
            commandList->dispatch(
                constants.viewSize.x / 16u + (constants.viewSize.x % 16u != 0u),
                constants.viewSize.y / 16u + (constants.viewSize.y % 16u != 0u));
