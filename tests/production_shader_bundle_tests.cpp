...
namespace
{
    ...
    std::set<std::string> GetExpectedShaderFiles()
    {
        ...
        constexpr const char* appShaders[] = {
            ...
            "smaa_edge_ps",
            "smaa_blend_weight_ps",
            "smaa_neighborhood_ps",
            "smaa_selective_neighborhood_cs",
...
int main(int argc, char** argv)
{
    ...
    constexpr const char* forbiddenShaders[] = {
        "smaa_edge_cs",
        "smaa_classify_tiles_cs",
        "smaa_selective_edge_cs",
        "smaa_selective_blend_weight_cs",
        "smaa_debug_ps"
    };
    ...
    passed &= Check(
        CountOccurrences(config, "TAA_EXPORT_SELECTIVE=1") == 3u &&
        ...
        "production TAA bundles must retain only shipping selective output without resurrection, debug, or pixel experiments");
    passed &= Check(
        config.find(
            "smaa_edge_ps.hlsl -T ps -E main -D SMAA_QUALITY_PRESET={0,1,2,3}") !=
                std::string::npos &&
            config.find(
            "smaa_blend_weight_ps.hlsl -T ps -E main -D SMAA_QUALITY_PRESET={0,1,2,3}") !=
                std::string::npos &&
            config.find(
            "smaa_neighborhood_ps.hlsl -T ps -E main") !=
                std::string::npos,
        "production must retain the fixed diagnostic SMAA quality pipeline without temporal or execution experiments");
    passed &= Check(
        CountOccurrences(config, "cmaa2.hlsl -T cs") == 4u &&
            CountOccurrences(
                config,
                "CMAA2_STATIC_QUALITY_PRESET={0,1,2}") == 4u,
        "production must retain all four official CMAA2 stages for Low, Medium, and High");
    passed &= Check(
        CountOccurrences(
            config,
