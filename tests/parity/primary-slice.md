...
T002/T003 source mapping and fixture freeze, 2026-09-19. source: AGFX `f91b108a111d2ca3ca4b6586b6cb5dd750064fd7`. [file and fixture hashes](primary-slice-sources.json) freeze this read scope. [E-028](../../docs/execution.md#e-028-2026-09-19-executed-the-direct-rust-buffer-copy) implements the direct Rust buffer-copy behavior and checks all frozen bytes in Debug/Release on Windows Vulkan. [E-029](../../docs/execution.md#e-029-2026-09-19-executed-the-ordinary-rust-compute-slice) also executes the four-pass Rust compute shader at opt0/opt3 with resource slots 1/3, checking the frozen selected bytes and every untouched descriptor buffer. duplicate C/Cpp/Ez registrations remain proposed equivalent mappings until their distinct lifetime/state assertions are accounted for. unchanged AGFX reference execution remains unavailable.
...
| `agfxBufferCreate`, `agfxBufferMap`, `agfxBufferUnmap`, `agfxBufferDestroy` | exact sizes, upload/GPU/readback memory roles, initialized upload bytes, mapped lifetime and completion before reuse | owned `Buffer` borrowing its device. checked ranges and explicit completion. implemented in `crates/agfx/src/vulkan.rs`, reviewed and executed under U-010 |
| `agfxBufferViewCreate`, `agfxBufferViewGetHandle` | raw writable view, source host handle returned as `uint64_t`, shader resource index as 32 bits | the bounded compute owner uses an explicitly checked u32 slot 0..3 at set0/binding0. the full source allocator and host64-bit view-handle API are not implemented. source samplers use binding1. this does not implement NGAPI heaps. [A-003/A-008](../../docs/agfx-port-notes.md) record handle and slot-allocation candidates |
| `agfxShaderModuleCreate`, `agfxComputePipelineCreate` | SPIR-V words, exact entry, stage and group size; shader module retired after pipeline creation | implemented ShaderCode words/stage/entry and BufferCompute ownership under U-011, with explicit fixture metadata and reviewed Rust payload hashes. the source HLSL remains the oracle |
| compute pass begin/end, pipeline binding, push constants, copy/dispatch, `BufferUAVBarrier` | 16-byte ordinary AGFX root: resource index, element count, pass index, padding. source state transitions and ordered dependent dispatches | BufferCompute::four_passes records four 64-lane dispatches, three whole-buffer UAV dependencies and final visibility. no render graph or generic backend layer |
...
the fixtures retain AGFX copyright 2026 Amélie Heinrich and the complete [MIT notice](../../legal/licenses/AGFX-MIT.txt). the Rust compute shader and copy fixture retain that attribution. the two bounded behaviors have direct Rust native execution. the wider source API and distinct Ez assertions remain separate work. no ShaderToHuman files or goldens are imported in this checkpoint.
