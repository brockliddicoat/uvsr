...
int main()
{
    ...
    const float activationRadius = 0.1f;
    const float3 activationCamera(-0.1f, 0.f, 0.f);
    const float3 activationDesired(0.3f, 0.f, 0.f);
    const float3 activationStart = wallWorld.ResolveSphere(
        activationCamera,
        activationDesired - activationCamera,
        activationRadius);
    const float3 activatedEmitter = wallWorld.MoveSphere(
        activationStart,
        activationDesired,
        activationRadius);
    passed &= Check(
        activatedEmitter.x <= -0.099f && activatedEmitter.x >= -0.102f,
        "first activation sweeps from the camera side instead of appearing through a thin wall");
    ...
    passed &= Check(sliding.x <= -0.249f && sliding.x >= -0.252f,
        "wall sliding maintains the mounted-object hitbox radius");
    ...
    passed &= Check(all(stationaryWithWallBetweenPivot == stationaryEye),
        "a stationary sphere retains the zero-cost idle path");
