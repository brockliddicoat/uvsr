...
int main(int argc, char** argv)
{
    ...
    for (uint32_t slot = 0u; slot < Components.size(); ++slot)
    {
        ...
        const std::string registerText = std::to_string(16u + slot);
        RequireContains(
            deferredShader,
            "texture2d<float>t_directionalvisibility" + slotText +
                ":register(t" + registerText + ");",
            "directional visibility slot " + slotText +
                " must retain its unique shader register");
        RequireContains(
            deferredShader,
        ...
        Require(
            CountOccurrences(
                deferredPass,
                "bindinglayoutitem::texture_srv(" + registerText + ")") ==
                1u,
            "directional visibility slot " + slotText +
                " must have exactly one CPU binding-layout entry");
        Require(
            CountOccurrences(
                deferredPass,
                "bindingsetitem::texture_srv(" + registerText + ",") ==
                1u,
            "directional visibility slot " + slotText +
                " must have exactly one CPU binding-set entry");
        RequireContains(
            deferredPass,
            "bindingsetitem::texture_srv(" + registerText +
                ",activevisibility[" + slotText +
                "].texture?activevisibility[" + slotText +
                "].texture:m_commonpasses->m_whitetexture.get())",
            "directional visibility slot " + slotText +
                " must fail open to the white texture");
    }
    ...
    RequireContains(
        ...
        "the direct-light loop must consume the exact-light visibility product");
    RequireContains(
        deferredPass,
    ...
    Require(
        CountOccurrences(
            rendererSource,
            "constdirectionallightvisibilitysetdirectionalvisibility=") == 1u,
    ...
    // This branch intentionally has no canonical-main MSAA or fused visibility
    // shader. Integration must add every imported production lighting variant
    // to this explicit list instead of weakening the invariant.
