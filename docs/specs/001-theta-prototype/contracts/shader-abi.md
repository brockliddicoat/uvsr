...
[E-026](../../../execution.md#e-026-2026-09-19-tested-divergent-native-heap-access) executes a separate four-lane variant. the root retains size 16 and offsets 0/8/12, with two XOR masks selecting which lanes use resource slots 1/3 and sampler slots 2/3. each lane writes a disjoint Vec4 at output offset 0/16/32/48. all four phases pass at opt0/opt3 in Debug/Release with validation. [U-007](../../../../UNSAFE.md#u-007-actual-ngapi-divergent-heap-sample) owns the contract, including the queried/enabled sampled-image non-uniform feature. additional resource types and stages remain open.

[E-027](../../../execution.md#e-027-2026-09-19-verified-the-native-heap-textured-cube) executes indexed vertex/fragment stages in the actual host. its separate root is 80 bytes, with a u64 vertex address at 0, four row-major float4 rows at 8 and resource/sampler u32 indices at 72/76. vertices are 24 bytes/align4 with position[4] at 0 and UV[2] at 16. opt0 reads the aggregate with Aligned4; opt3 splits it into six scalar Aligned4 reads. both retain implicit fragment sampling from native heaps. [U-008](../../../../UNSAFE.md#u-008-actual-ngapi-textured-cube) owns this diagnostic adaptation and its color/depth oracle.
