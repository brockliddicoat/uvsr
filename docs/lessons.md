...
## L-014. isolate native pipeline compilation from shader execution

**observed, task/mesh fixture on one NVIDIA driver.** at [E-033](execution.md#e-033-2026-09-19-executed-task-and-mesh-stages), a valid PhysicalStorageBuffer64 Vulkan module caused an access violation in the native pipeline compiler before any case completed. separate-stage controls isolated the original mesh body. changing only four bounded Output-array indices from u64 to u32 in a diagnostic module allowed pipeline creation while preserving physical addressing. this is evidence about the trigger, not a complete driver root-cause proof.

application: separate source compilation, module validation, pipeline creation and actual draw/readback. preserve original artifacts and use bounded controls that submit no GPU work while isolating a pipeline fault. a source fixture with six one-quad groups and fixed output slots then passed the unchanged numeric cube oracle in all four compiler configurations and both hosts. keep that narrower contract explicit instead of truncating physical addresses or silently claiming dynamic-index support. limit: no general driver defect, performance improvement or other-driver result is established. abnormal process exit cannot prove zero shader execution merely because no completed case record was printed.

## L-015. include forgotten children and allocation limits in native ownership

**observed by source review of the Rust port.** E-038 found that a borrowed resource can be consumed by safe `mem::forget`, allowing its Device to drop without the native child being destroyed. lifetimes alone do not discharge Vulkan's parent-destruction precondition. the initial allocation path also relied on native allocation failure without enforcing maxMemoryAllocationCount, whose limit the Vulkan memory rules require the application to respect.

application: count private resource leases before native creation, release after native cleanup, and preserve the parent/loader if a forgotten child stays counted. share the device allocation budget across all resource types and check allocation-size, heap-size and buffer-size limits before their native operations. tests cover rollback, capacity and forgotten leases using safe CPU state, without exhausting a GPU or issuing invalid calls. limit: this closes the identified paths in the current synchronous owner. it does not prove every future resource owner or arbitrary native escape sound. [U-010](../UNSAFE.md#u-010-rust-vulkan-device-buffer-and-synchronous-submission) records the exact implementation and normative sources.


## L-016. keep evidence parsing linear in record size

**observed in E-043.** the native raster fixture finished100images and26depth readbacks in about four seconds, but an unanchored validation regex repeatedly scanned suffixes of its long JSON line. the coordinator stopped that CPU parser after more than87seconds of recorded process CPU time. its218158-character captured output scans in about0.0017seconds with a line split and a simple marker search. the same ten intentional loader notices remain separately classified, with no broader diagnostic filter.

application: split structured result and diagnostic streams by lines before searching them. retain a large ordinary-record control beside warning/error/notice controls, and preserve captures when reporting fails after native work. limit: these are one local parser measurements, not renderer performance measurements. the interrupted report did not persist its native exit code, so fresh final gates establish the reported execution result. no shader, image or validation rule was weakened.

## L-017. make execution depend on a successful review gate

**observed at E-044.** an automated review-record script incorrectly expected a storage image's SPIR-V Depth operand to be0 instead of the valid unknown value2. its assertion stopped record creation, but the next tool command still launched the native runner. source/caller inspection and independent module validation preceded the run, and its bounded outputs passed. those facts do not turn the missing pre-execution record into a passed gate.

application: check the returned process exit status before every dependent command, and persist the review record inside the same successful control flow that authorizes execution. a successful tool transport is not a successful subprocess. preserve the timing limitation when a gate is completed afterward. the subsequent original-source and Release runs used explicit successful identity gates. this is a workflow finding, not evidence of a GPU defect or a reason to ask again for already-authorized work.

## L-018. include floating-point execution modes in source parity

**observed at E-047.** Features intentionally draws zero-area arrowheads. HLSL saturation defines a NaN input as zero, while Rust clamp propagates it. restoring that scalar rule made CPU contracts pass, but the Vulkan image still lost complete arrow rows and the centered-mouse case. requesting float32 SignedZeroInfNanPreserve on the image and state-update entries restored the rows in both shader optimization levels. the host queries the selected device's property and rejects unsupported execution before creating those pipelines. no new RustGPU compiler patch was required.

the [Vulkan floating-point rules](https://docs.vulkan.org/spec/latest/appendices/spirvenv.html#spirvenv-fp-operation) permit NotNaN/NotInf assumptions when the preservation mode is absent. source-level NaN checks alone therefore do not establish the intended GPU behavior. preservation also does not disable expression rearrangement. DXC Gis adds a stricter control, and its NoContraction differences still matter when comparing sensitive expressions. retain compiler flags, emitted modes, device properties, source inputs and failed captures together. the resulting Features checks establish bounded behavior on the tested device, not cross-driver bit identity or complete image parity.
