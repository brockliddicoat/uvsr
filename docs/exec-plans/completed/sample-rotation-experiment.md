...
- Archived path: `docs/exec-plans/completed/sample-rotation-experiment.md`

## Goal and Done Condition

Goal: Implement a conservative, default-off receiver-rotation experiment for UVSR's supported one-sample-per-2x2 visibility layout without increasing the per-frame sample budget or changing unsupported/native behavior.
...
| 2026-07-15 | Preserve a logical block-history lattice. | AO/GI, depth/normal history, and adaptive feedback are sampling-resolution resources. The experiment can average receiver phases into one block estimate without new resources, but cannot retain four independent output pixels. | Reprojection, filtering, and limitations |
