...
the product is a lightweight explicit Rust graphics framework with native D3D12, Vulkan, and Metal 4 backends. AGFX is an inspiration for explicit devices, queues, command recording, resource states, handles, capabilities, backend separation, shader ABI, and test-facing behavior. this project owns its public contracts and does not promise drop-in AGFX compatibility.

use explicit ownership and small unsafe boundaries around D3D12, Vulkan, Metal, windowing, and compiler interfaces. resource destruction follows backend completion, not Rust lexical scope alone. record each public behavior, native difference, and portability limit with its reason and test.
