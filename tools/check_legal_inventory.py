#!/usr/bin/env python3
"""Validate UVSR's legal folder, license text, and source-record index."""
...
SOURCE_ROOT = ROOT / "legal" / "sources"
REQUIRED_NOTICE = (
    "Required Notice: Copyright 2026 Brock Liddicoat. "
    "UVSR: https://github.com/brockliddicoat/uvsr"
)
...
def check_layout(paths: set[str]) -> None:
    ...
    for required in (
        ...
        "legal/code-samples/README.md",
        "legal/documentation/COMMERCIAL-LICENSING.md",
        "legal/sources/README.md",
    ):
        if required not in paths:
            fail(f"required legal entry is missing: {required}")


def check_source_index() -> None:
    index_path = SOURCE_ROOT / "README.md"
    ...
    for target in MARKDOWN_LINK.findall(index):
        ...
        if not clean or "://" in clean or clean.startswith("mailto:"):
            continue
        resolved = (SOURCE_ROOT / clean).resolve()
        try:
        ...
        if len(relative.parts) == 1 and relative.suffix.casefold() == ".md":
            ...
            if not resolved.is_file():
                fail(f"source index contains a broken link: {clean}")
    missing = sorted(records - linked, key=str.casefold)
    if missing:
        fail("source records missing from index: " + ", ".join(missing))
