...
def main():
    ...
    parser.add_argument('--views', action='store_true', help='compile the format-view compute/raster controls')
    args = parser.parse_args()
    upstream = args.rustgpu_source.resolve(strict=True)
    backend = args.codegen_backend.resolve(strict=True)
    output = args.output_dir.resolve()
    output.mkdir(parents=True, exist_ok=True)
    name = 'texture_views' if args.views else 'raster'
    ...
    for level in (0, 3):
        ...
        forbidden = r'PhysicalStorageBuffer|ResourceHeapEXT|SamplerHeapEXT' + ('' if args.views else r'|OpTypeImage|OpTypeSampler')
        ...
        record = dict(schema_version=1, case_id=f'agfx.{name}.compile.opt{level}', status='pass',
            language='Rust', stage='vertex+fragment+compute' if args.views else 'vertex+fragment', entries=entries,
            target='spirv-unknown-vulkan1.3', profile='agfx-format-views' if args.views else 'agfx-ordinary-raster', payload_type='SPIR-V',
