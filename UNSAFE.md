...
| area | observed state |
| --- | --- |
| first-party Theta Rust | the compiler diagnostic module uses `forbid(unsafe_code)`. the actual-NGAPI shader uses deny lints with U-002's narrow exception. no Cargo workspace or Rust runtime port exists. the contributed compile-only shader fixture remains U-001 |
| native and shader boundaries | U-001 is compile-only. U-002 covers the scalar runtime shader and its actual NGAPI diagnostic caller, reviewed and executed in Debug/Release |
...
| RustGPU contribution | E-015 passed all 16 assembly-input probes in both tool configurations. E-018 adds aligned scalar accesses. E-019 adds qptr memory effects with a native optimizer prerequisite. E-020 adds address-only comparisons and wrapping offsets. U-001 has an unsafe entry and three raw accesses with one narrow lint exception. compilation does not establish its runtime obligations |
| runtime and soundness evidence | E-021 passes six scalar Rust shader cases each in actual NGAPI Debug/Release with explicit validation. native heap lookups and nonzero high-address runtime coverage remain open. this is bounded execution evidence, not a Rust boundary soundness proof |
...
status: reviewed and executed for this bounded dispatch. [E-021](docs/execution.md#e-021-2026-09-19-executed-rust-physical-readback-through-ngapi) records the exact runtime evidence and limits.

| field | record |
| --- | --- |
| identity and scope | [physical_readback.rs](tools/theta/ngapi-probe/physical_readback.rs)::`computeMain`, its one unsafe entry declaration, four explicit raw access blocks and one narrow lint exception. [physical_readback.cpp](tools/theta/ngapi-probe/physical_readback.cpp)::`run_variant` is the only runtime caller through actual NGAPI. the [shared host layout](tools/theta/ngapi-probe/physical_readback.hpp) contains two u64 addresses. generated metadata records the exact shader source, module, compiler and library hashes. |
...
| change history | introduced for the first real-address NGAPI consumer after E-020. first native attempts stopped before dispatch on the initial high-address coverage check. the detailed allocation record established valid low addresses, so review separated that coverage condition from memory validity. the shader contract and all range/synchronization checks remain unchanged. this boundary is separate from compile-only U-001 and generic compiler patches. allocation, layout, dispatch count, synchronization, shader or host dependency changes require renewed review. |
