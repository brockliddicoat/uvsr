...
[rustgpu-native-heap-library.patch](rustgpu-native-heap-library.patch) follows the debug-stripping fixture patch and retains MIT OR Apache-2.0 terms. two unsafe associated constructors return existing Image and Sampler values using native type sizes and explicit heap access. this needs no new compiler intrinsic, ownership abstraction or host dependency. source docs carry the complete descriptor, lifetime, synchronization and feature contract. CPU stubs panic, and safe constructor calls are rejected.
...
[rustgpu-storage-images.patch](rustgpu-storage-images.patch) follows the pointer-contract checkpoint. one generic rgba32ui shader reads a native storage image, writes a texel in a separate image and writes the changed value to physical memory. four source wrappers cover default/qptr and opt0/opt3. the complete unsafe contract stays in the shared body. no compiler or library implementation changes, new public API or host dependency is added.

all 47 required pointer/heap source pairs pass, seven logical and 40 physical64. linked expectations and final independently validated modules retain the native heap/size/stride declarations, root offsets, ImageRead/ImageWrite and aligned physical output. [E-032](../../docs/execution.md#e-032-2026-09-19-executed-native-storage-image-operations) records16 actual NGAPI cases per Debug/Release host with full readback and zero validation diagnostics. [U-013](../../UNSAFE.md#u-013-native-storage-image-fixture) records the review. this is one storage format and one-invocation access contract, with broader resource/stage and upstream CI claims still pending.
