...
apply [ngapi-task-feature.patch](ngapi-task-feature.patch) after the storage-image patch with `git apply --unidiff-zero`. [task-source.json](task-source.json) pins its base, hash and reconstructed tree. the single added line requires taskShader during device selection, matching the feature NGAPI already enables. it adds no optional mode or enabled feature. [E-033](../../docs/execution.md#e-033-2026-09-19-executed-task-and-mesh-stages) records the task/mesh consumer. this host prerequisite remains separate from RustGPU and has not been submitted upstream.
