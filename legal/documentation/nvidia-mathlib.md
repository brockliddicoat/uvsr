...
the former integration fetched the exact commit archive directly. The commit plus archive hash is
the immutable trust boundary; `v11` is descriptive. Former NRD C++ and HLSL
included MathLib headers; UVSR does not maintain an alternate or transitive
...
The former production mapping copied that exact file to
...
portions. The former package mapping preserved the complete upstream text; it does not
change those terms.
